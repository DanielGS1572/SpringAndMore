	...
	init: function (editor) {
		editor.addCommand( 'validaContenido', {
			...
			exec: function( editor ) {
				...
				//var element = CKEDITOR.dom.element.createFromHtml( "<span style='background-color:cyan'> xxx </span>" );
				//editor.insertElement( element );
				
													//editor.editable().setHtml("<span style='background-color:cyan!important'>ixxxpsum</span>");

			/*	for(i=0; i < 10; i++){
					var element = CKEDITOR.dom.element.createFromHtml("<span style='background-color:cyan'> " + i +  " </span>" );
					editor.insertElement( element );
				}*/
					//editor.insertHtml("<h2>hey</h2>");

				//editor.setHtml(editor.getData().replace("contenido","<span style='background-color:cyan'> xxx </span>"));
				//editor.setHtml(writer);
